...
- **Clean separation**: The `.ai/` directory is its own repo with its own history. It doesn't pollute the host project's
  log, and removing it is a clean operation.
...
The main tradeoff is that submodules require contributors to run `git submodule update` (or clone with
  `--recurse-submodules`). This is a well-understood git workflow and the commands are documented below.
